...
bool Server::operator>(Server server)
{
	if (this->getBrand() > server.getBrand())
		return true;
}
...
fstream& operator>>(fstream& in, Server& server)
{
	in >> *(dynamic_cast<Static*>(&server));
	in >> server.memHD;
	return  in;
}

fstream& operator<<(fstream& out, Server& server)
{
	out << *(dynamic_cast<Static*>(&server));
	out << server.memHD << "\n";
	return  out;
}

ifstream& operator>>(ifstream& in, Server& server)
{
	in >> *(dynamic_cast<Static*>(&server));
	in >> server.memHD;
	return  in;
...
ofstream& operator<<(ofstream& out, Server& server)
{
	out << *(dynamic_cast<Static*>(&server));
	out << server.memHD << "\n";
	return  out;
